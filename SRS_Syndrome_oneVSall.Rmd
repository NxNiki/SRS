...
label = c("ASD", "DS", "SMS", "WS", "+1X", "+1X+1Y", "+1Y")
...
for (i in 1:length(label)){
    ...
    coefs = coef(cv.fit, s = cv.fit$lambda.min)
    coefs_list[[i]] = coefs
...
```{r}
print(result)
write.table(result, 'classification_one_vs_all.csv', sep = ',', row.names = F)

colnames(result_coefs) = label[1:7]
print(result_coefs)
write.csv(result_coefs, 'classification_one_vs_all_coefs.csv', row.names = T)
```
