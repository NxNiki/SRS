...
for (i in 1:length(label)){
    ...
    set.seed(123)
    cv.fit =  cv.gglasso(x.train.sort, y.train, nfolds=5, group = group.sort$x,loss="logit", pred.loss="misclass", lambda.factor = .05)
    ...
    myacc<-function(ypred,ytrue){
        # this function does not work when ypred or ytrue only has one values.
...
```{r}
print(result)
write.table(result, 'classification_one_vs_all_updated2.csv', sep = ',', row.names = F)

colnames(result_coefs) = label
print(result_coefs)
write.csv(result_coefs, 'classification_one_vs_all_coefs_updated2.csv', row.names = T)
```
...
```{r}
print(result)
write.table(result, 'classification_one_vs_all_lasso.csv', sep = ',', row.names = F)

colnames(result_coefs) = label
print(result_coefs)
write.csv(result_coefs, 'classification_one_vs_all_coefs_lasso.csv', row.names = T)
```
