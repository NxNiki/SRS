...
for (i in 1:length(label)){
    ...
    cv.fit =  cv.gglasso(x.train, y.train, nfolds=5, group = group,loss="logit", pred.loss="misclass", lambda.factor=0.03)
...
```{r}
print(result)
write.table(result, 'classification_one_vs_all_0.03.csv', sep = ',', row.names = F)

colnames(result_coefs) = label
print(result_coefs)
write.csv(result_coefs, 'classification_one_vs_all_coefs_0.03.csv', row.names = T)
```
